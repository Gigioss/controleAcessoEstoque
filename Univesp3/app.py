...
@app.route('/api/registro-bidirecional', methods=['POST'])
def registro_bidirecional():
    try:
        data = request.json
        tipo = data.get('tipo')
        registros_anteriores = len(detector.current_registros)
        
        if tipo == "entrada":
            # Verifica se apareceu nova pessoa na câmera
            if len(detector.current_registros) > registros_anteriores:
                return jsonify({
                    'status': 'confirmed',
                    'message': 'Nova pessoa detectada pela câmera'
                })
            return jsonify({
                'status': 'unconfirmed',
                'message': 'Nenhuma nova pessoa detectada'
            })
            
        elif tipo == "saida":
            # Verifica se alguém saiu
            if len(detector.current_registros) < registros_anteriores:
                return jsonify({
                    'status': 'confirmed',
                    'message': 'Pessoa saiu do campo de visão'
                })
            return jsonify({
                'status': 'unconfirmed',
                'message': 'Nenhuma pessoa saiu'
            })
